...
class ParquetRecordBatchReader : public arrow::RecordBatchReader {
 public:
  ...
  arrow::Status ReadNext(std::shared_ptr<arrow::RecordBatch> *batch) override {
    *batch = nullptr;
    ...
    if (!with_cache_) {
      return stream_uncached_ ? ReadNextUncached(batch) : ReadRowGroup(batch);
    }
 ...
 private:
  ...
  arrow::Status ReadRowGroup(std::shared_ptr<arrow::RecordBatch> *batch) {
  ...
  arrow::Status ReadNextUncached(std::shared_ptr<arrow::RecordBatch> *batch) {
