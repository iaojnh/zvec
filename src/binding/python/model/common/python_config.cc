...
void ZVecPyConfig::Initialize(pybind11::module_ &m) {
  m.def("Initialize", [](py::args args, py::kwargs kwargs) -> py::none {
    ...
    const auto read_partition_mb = [&config_dict](const char *key,
                                                  uint64_t *out) {
      ...
      if (mb < 0) {
        throw py::value_error(std::string(key) + " must be non-negative");
      }
    ...
    read_partition_mb("resident_metadata_mb", &data.resident_metadata_bytes);
