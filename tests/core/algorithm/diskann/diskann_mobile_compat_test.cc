...
template <typename T>
void ExpectLegacyPqChunkWidths(IndexMeta::DataType data_type) {
  constexpr uint32_t kDimension = 5;
  constexpr uint32_t kCenterCount = 256;
  constexpr uint32_t kChunkCount = 2;
  const std::array<uint32_t, kChunkCount + 1> chunk_offsets{{0, 3, 5}};

  std::vector<T> pivots(kCenterCount * kDimension);
  for (size_t i = 0; i < pivots.size(); ++i) {
    pivots[i] = T(static_cast<float>(i + 1));
  }

  // Legacy indexes store full pivot rows. Import them through the new
  // quantizer and check that uneven chunks retain their exact components.
  std::string payload(reinterpret_cast<const char *>(pivots.data()),
                      pivots.size() * sizeof(T));
  payload.append(kDimension * sizeof(T), '\0');
  payload.append(reinterpret_cast<const char *>(chunk_offsets.data()),
                 sizeof(chunk_offsets));

  IndexMeta meta(data_type, kDimension);
  meta.set_metric("SquaredEuclidean", 0, ailego::Params());
  auto quantizer = DiskAnnUtil::create_quantizer_from_meta_buffer(
      payload, meta, kChunkCount, true);
  ASSERT_NE(quantizer, nullptr);
  ASSERT_EQ(quantizer->quantized_datapoint_vector_length(), kChunkCount);

  const std::array<uint8_t, kChunkCount> code{{1, 255}};
  IndexQueryMeta query_meta(IndexMeta::DataType::DT_FP32, kDimension);
  std::string decoded;
  ASSERT_EQ(quantizer->dequantize(code.data(), query_meta, &decoded), 0);
  ASSERT_EQ(decoded.size(), kDimension * sizeof(float));

  std::array<float, kDimension> actual{};
  std::memcpy(actual.data(), decoded.data(), decoded.size());
  const std::array<float, kDimension> expected{{6, 7, 8, 1279, 1280}};
  for (size_t i = 0; i < actual.size(); ++i) {
    EXPECT_FLOAT_EQ(actual[i], expected[i]);
  }
}

TEST(DiskAnnMobileCompatTest, LegacyPqImportPreservesExactChunkWidths) {
  ExpectLegacyPqChunkWidths<float>(IndexMeta::DataType::DT_FP32);
  ExpectLegacyPqChunkWidths<ailego::Float16>(IndexMeta::DataType::DT_FP16);
}
